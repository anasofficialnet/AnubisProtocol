...
# SSH helper using sshpass-equivalent
...
string af = Path.GetTempFileName() + ".cmd";
File.WriteAllText(af, "@echo " + pw);
var si = new ProcessStartInfo("ssh");
si.Arguments = "-o StrictHostKeyChecking=no -o ConnectTimeout=60 -o ServerAliveInterval=30 -p " + port + " root@127.0.0.1 " + cmd;
si.UseShellExecute = false;
si.RedirectStandardInput = true;
si.RedirectStandardOutput = true;
si.RedirectStandardError = true;
si.CreateNoWindow = true;
si.Environment["SSH_ASKPASS_REQUIRE"] = "force";
si.Environment["SSH_ASKPASS"] = af;
si.Environment["DISPLAY"] = ":0";
var p = Process.Start(si);
p.StandardInput.Close();
bool ok = p.WaitForExit(timeout);
string o = ok ? p.StandardOutput.ReadToEnd() : "";
string e = ok ? p.StandardError.ReadToEnd() : "";
int code = ok ? p.ExitCode : -1;
try { File.Delete(af); } catch {}
return "EXIT:" + code + "|" + o + e;
}
public static void Send(string remoteCmd, string localFile, string pw, int port, int timeout){
string af = Path.GetTempFileName() + ".cmd";
File.WriteAllText(af, "@echo " + pw);
var si = new ProcessStartInfo("ssh");
si.Arguments = "-o StrictHostKeyChecking=no -o ConnectTimeout=60 -o ServerAliveInterval=30 -p " + port + " root@127.0.0.1 " + remoteCmd;
si.UseShellExecute = false;
si.RedirectStandardInput = true;
si.RedirectStandardOutput = true;
si.RedirectStandardError = true;
...
function S([string]$cmd, [int]$t=60000) {
    $r = [SSH]::Run("""$cmd""", "toor", $script:sshPort, $t)
    if ($r -match "^EXIT:0") { Write-Host "  OK: $cmd" -F Green }
    else { Write-Host "  FAIL: $cmd => $($r.Substring(0,[Math]::Min(80,$r.Length)))" -F Red }
    return $r
}

# Keyboard scancode helpers for live CD
function TT($t){$m=@{'a'=0x1e;'b'=0x30;'c'=0x2e;'d'=0x20;'e'=0x12;'f'=0x21;'g'=0x22;'h'=0x23;'i'=0x17;'j'=0x24;'k'=0x25;'l'=0x26;'m'=0x32;'n'=0x31;'o'=0x18;'p'=0x19;'q'=0x10;'r'=0x13;'s'=0x1f;'t'=0x14;'u'=0x16;'v'=0x2f;'w'=0x11;'x'=0x2d;'y'=0x15;'z'=0x2c;'1'=0x02;'2'=0x03;'3'=0x04;'4'=0x05;'5'=0x06;'6'=0x07;'7'=0x08;'8'=0x09;'9'=0x0a;'0'=0x0b;'-'=0x0c;'='=0x0d;' '=0x39;'.'=0x34;'/'=0x35;','=0x33};$s=@{'!'=0x02;'@'=0x03;'#'=0x04;'$'=0x05;'%'=0x06;'^'=0x07;'&'=0x08;'*'=0x09;'('=0x0a;')'=0x0b;'_'=0x0c;'+'=0x0d;'|'=0x2b;':'=0x27;'"'=0x28;'>'=0x34;'<'=0x33;'?'=0x35};foreach($c in $t.ToCharArray()){$k=$c.ToString().ToLower();$u=($c -cmatch '[A-Z]');$al=@();if($u -and $m.ContainsKey($k)){$al+=@(0x2a,$m[$k])}elseif($s.ContainsKey([string]$c)){$al+=@(0x2a,$s[[string]$c])}elseif($m.ContainsKey([string]$c)){$al+=@($m[[string]$c])};if($al.Count -gt 0){$sc=@();foreach($x in $al){$sc+=("{0:x2}" -f $x)};foreach($x in $al){$sc+=("{0:x2}" -f ($x+0x80))};& $VBox controlvm $vmName keyboardputscancode $sc 2>$null};Start-Sleep -Milliseconds 80}}
function PR{& $VBox controlvm $vmName keyboardputscancode 1c 9c 2>$null;Start-Sleep -Milliseconds 500}
function T($t,$w=3){TT $t;PR;Start-Sleep -Seconds $w}
...
if (!(Test-Path $iso)) { Write-Host "  ERROR: ISO not found at $iso"; exit 1 }
...
& $VBox modifyvm $vmName --memory 1024 --cpus 1 --audio-driver none --nic1 nat 2>&1 | Out-Null
& $VBox modifyvm $vmName --natpf1 "ssh,tcp,,$sshPort,,22" 2>&1 | Out-Null
...
Write-Host "  Booting 50s..."; Start-Sleep 50

#====================================================================
Write-Host "`n[2/8] LIVE CD SSH" -F Cyan
#====================================================================
TT "root"; PR; Start-Sleep 5
T "echo root:toor | chpasswd" 3
T "ifconfig eth0 up" 3
T "udhcpc -i eth0" 5
T "apk add openssh-server" 15
T "ssh-keygen -A" 5
...
T "/usr/sbin/sshd" 5
ssh-keygen -R "[127.0.0.1]:$sshPort" 2>$null
$r = S "hostname" 30000
if ($r -notmatch "^EXIT:0") { Write-Host "SSH FAILED!"; exit 1 }
...
Write-Host "  setup-disk ~10min..."
S "echo y | setup-disk -m sys /dev/sda 2>&1 | tail -3" 900000

#====================================================================
Write-Host "`n[4/8] CONFIGURE" -F Cyan
#====================================================================
S "mount /dev/sda3 /mnt 2>/dev/null; mount /dev/sda1 /mnt/boot 2>/dev/null; mount -o bind /dev /mnt/dev; mount -o bind /proc /mnt/proc; mount -o bind /sys /mnt/sys; echo mounts-ok"
S "printf 'auto lo\niface lo inet loopback\n\nauto eth0\niface eth0 inet dhcp\n\nauto eth1\niface eth1 inet dhcp\n' > /mnt/etc/network/interfaces"
S "echo anubis > /mnt/etc/hostname"
S "echo nameserver 8.8.8.8 > /mnt/etc/resolv.conf"
...
# IP display
$ipB64 = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes(@'
#!/bin/sh
sleep 5
...
& $VBox startvm $vmName --type headless 2>&1 | Out-Null
Write-Host "  60s boot..."; Start-Sleep 60
ssh-keygen -R "[127.0.0.1]:$sshPort" 2>$null
$r = S "hostname" 120000
if ($r -notmatch "^EXIT:0") { Write-Host "BOOT FAIL!"; exit 1 }
Write-Host "  DISK OK!" -F Green
...
#====================================================================
& $VBox controlvm $vmName poweroff 2>$null; Start-Sleep 15
...
if (Test-Path $ova) {
    ...
    Write-Host "`n  OVA: ${sz}MB" -F Green
} else { Write-Host "  EXPORT FAILED!"; exit 1 }
& $VBox unregistervm $vmName --delete 2>$null
Remove-Item $buildDir -Recurse -Force -ErrorAction SilentlyContinue
Write-Host "`n  BUILD COMPLETE!" -F Green
